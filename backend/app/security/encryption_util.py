...
def decrypt_field(ciphertext_b64: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        if len(raw) < 28:  # 12-byte nonce + minimum 16-byte GCM tag
            raise ValueError("Invalid ciphertext length: must contain at least 12-byte nonce and 16-byte tag.")
        ...
        raise ValueError(f"AES-256 decryption failed (tampered ciphertext or invalid key): {e}")

def verify_ciphertext_integrity(ciphertext_b64: Optional[str]) -> bool:
    """
    Validates ciphertext integrity without raising an unhandled exception.
    """
    if not ciphertext_b64:
        return False
    try:
        dec = decrypt_field(ciphertext_b64)
        return dec is not None and len(dec) > 0
    except Exception:
        return False
...
def mask_gstin(gstin: Optional[str]) -> Optional[str]:
    """Masks GSTIN: 07ABCDE1234F1Z5 -> 07ABC******1Z5"""
...
def mask_udyam(udyam: Optional[str]) -> Optional[str]:
    """Masks Udyam: UDYAM-DL-01-0029145 -> UDYAM-DL-**-***9145"""
    if not udyam or not udyam.startswith("UDYAM"):
        return udyam
    parts = udyam.strip().split("-")
    if len(parts) >= 4:
        return f"{parts[0]}-{parts[1]}-**-***{parts[3][-4:]}"
    return udyam
